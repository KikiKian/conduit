...
func SaveConfig(targets []WatchTarget) error {
	if len(targets) == 0 {
...
func LoadConfig(overrides []string) (Config, error) {
	var config Config
	...
	for _, override := range overrides {
		...
		config.Targets = append(config.Targets, WatchTarget{
			Lang:     parts[0],
			FilePath: parts[1],
		})
	}
...
func NewEntryFromReflect(name string, variable any) error {
	varType := reflect.TypeOf(variable).String()
	if _, ok := typeMap[varType]; !ok {
		return fmt.Errorf("unsupported type %q", varType)
	}
	return AddEntry(name, varType, variable)
}
