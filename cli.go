		...
		case stepFilePaths:
			...
			case "enter":
				...
				if m.inputIdx >= len(m.inputKeys) {
					...
					if err := SaveConfig(m.targets); err != nil {
						m.err = err.Error()
						return m, nil
					}
					m.step = stepDone
	...
	if final.step == stepDone {
		...
		fmt.Println(boxStyle.Render(
			successStyle.Render("  conduit.config.json created!\n\n") +
				...
				warningStyle.Render("    1. ") + white.Render("conduit add") + dimStyle.Render(" — add your first variable\n") +
				warningStyle.Render("    2. ") + white.Render("conduit watch") + dimStyle.Render(" — start watching for changes\n") +
				warningStyle.Render("    3. ") + dimStyle.Render("use ") + white.Render("# conduit:import <name>") + dimStyle.Render(" in your files"),
		))
		fmt.Println()
	}
}
	...
	if m.field == fieldConfirm {
		b.WriteString("\n" + boxStyle.Render(
			dimStyle.Render("adding: ")+
				...
				dimStyle.Render(" = ")+white.Render(m.valInput.Value()),
		) + "\n")
...
func runAdd() {
	...
	if final.done {
		...
		fmt.Println(successStyle.Render("  ✓ ") +
			white.Render(final.nameInput.Value()) +
			dimStyle.Render(" ("+supportedTypes[final.typeCursor]+")") +
			dimStyle.Render(" = ") +
			white.Render(final.valInput.Value()) +
